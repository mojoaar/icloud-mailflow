...
- `/activity` Refresh now shows a spinner and a "Activity refreshed" toast on completion
...
- MCP rate limiter now periodically evicts expired entries (previously grew unbounded)
...
- Webhook secret in `/settings` now has a Generate button to create random 32-byte hex keys
